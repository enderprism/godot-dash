...
func _ready() -> void:
	...
	connect("button_up", self, "_button_unheld")

func _button_held() -> void:
	var scale_tween = create_tween()
	scale_tween.set_ease(Tween.EASE_OUT)
	scale_tween.set_trans(Tween.TRANS_BOUNCE)
	scale_tween.tween_property(self, "rect_scale", Vector2(1.1, 1.1), 0.2)

func _button_unheld() -> void:
	var scale_tween = create_tween()
	scale_tween.set_ease(Tween.EASE_OUT)
	scale_tween.set_trans(Tween.TRANS_BOUNCE)
	scale_tween.tween_property(self, "rect_scale", Vector2(1.0, 1.0), 0.2)
	if _switch_scene:
