...
func _ready() -> void:
	connect("spider_orb_entered", Callable(player, "_on_SpiderOrb_area_entered").bind(reverse, opposite_gravity)) # connect orb function to player
	connect("spider_orb_exited", Callable(player, "_on_SpiderOrb_area_exited")) # connect orb function to player
