...
func _ready() -> void:
	_setVars()
	connect("area_entered", Callable(player, signal_receptor_entered).bind(reverse)) # connect orb function to player
	connect("area_exited", Callable(player, signal_receptor_exited)) # connect orb function to player
