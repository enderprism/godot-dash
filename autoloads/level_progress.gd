...
func _physics_process(delta: float) -> void:
	if has_node("/root/Scene"):
		var _current_level_path = "/root/Scene/Levels/"+CurrentLevel.current_level
		var _current_level = get_node(_current_level_path)
		if _current_level.time_based_percentage && end_time > 0.0:
			calculated_percentage = (CurrentLevel.level_time*100) / end_time
			rounded_percentage = stepify(calculated_percentage, 1.0)
